...
impl Process {
    ...
    /// as the `Process` struct is dropped.
    #[inline(always)]
...
mod tests {
    ...
    fn iter_handles() -> Result<()> {
        ...
        let process = Process::open_first_named("Discord.exe", access)?;
    ...
    fn times() -> Result<()> {
        ...
        assert_eq!(
            ...
            "exited at time should be 0"
        );
        assert!(
            times.created_at > core::time::Duration::ZERO,
            "creation time shouldn't be 0"
        );
